...
### 必需规范（写进规则）
- 事件监听一律使用 `useEventListener`（避免内存泄漏与残留监听）
- 高频滚动/resize 回调必须节流（`useThrottleFn` 或 `watchThrottled`）
- 多行文本输入优先使用 `useTextareaAutosize`
- 弹窗/浮层高度优先使用 `useElementSize` / `useResizeObserver` 做动态适配

### 推荐规范（建议项）
- 列表项 > 6 时建议使用 `useVirtualList`
- 复杂输入/搜索建议使用 `watchDebounced`
- 需要复制时优先使用 `useClipboard`
