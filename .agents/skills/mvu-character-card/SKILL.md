...
当玩家要求编写 MVU 角色卡的脚本、前端界面时, 除了参考`初始模板/脚本`或`初始模板/前端界面`外, 你还应该参考`初始模板/角色卡`中的脚本和前端界面模板.同时读取进一步关于zod mvu结构的详细说明：`.cursor\rules\MVU组件包`
...
例如, `示例/角色卡示例/store.ts` 中定义了访问 `useDataStore` 函数:
...
除了`[initvar]变量初始化勿开`外, 世界书里还应该有`[mvu_update]变量列表`、`[mvu_update]变量更新规则`、`[mvu_update]变量输出格式`三个条目. 其中`变量列表`和`[mvu_update]变量输出格式`完全固定, 如果还没有设定, 可以按照初始模板中的设置.
...
`[mvu_update]变量输出格式`必须按照以下最新模板编写:
