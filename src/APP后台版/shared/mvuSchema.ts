...
const ModeSchema = z.preprocess(
  value => {
    ...
    if (normalized === 'play' || normalized === 'app') return 'app';
    if (normalized === 'rp' || normalized === 'roleplay') return 'rp';
    if (normalized === 'mixed' || normalized === 'mix') return 'mixed';
    return value;
  },
  z.enum(['app', 'rp', 'mixed']),
);
