<template>
  ...
  >
    ...
    <div
      class="pointer-events-none absolute inset-0 z-0 bg-[radial-gradient(circle_at_top,transparent_0%,var(--background)_70%)]"
    ></div>

    <div
      class="hud-deco pointer-events-none absolute left-4 top-20 bottom-24 z-10 hidden w-12 flex-col justify-between py-4 2xl:flex"
    >
      <div class="w-full h-32 border-l-2 border-t-2 border-primary/40 clip-corner-sm relative">
        ...
        <div
          class="absolute -right-4 top-0 font-mono text-[8px] text-primary/60 tracking-widest"
    ...
    <div
      class="hud-deco pointer-events-none absolute right-4 top-20 bottom-24 z-10 hidden w-12 flex-col items-end justify-between py-4 2xl:flex"
    >
      <div class="w-full h-32 border-r-2 border-t-2 border-primary/40 clip-corner-sm relative">
        ...
        <div
          class="absolute -left-4 top-0 font-mono text-[8px] text-primary/60 tracking-widest"
    ...
    <TopBar v-model:density="density" v-model:theme="theme" @open-sidebar="isSidebarOpen = true" />

    <CharacterSidebar :isOpen="isSidebarOpen" @toggle="isSidebarOpen = !isSidebarOpen" @selectChar="handleSelectChar" />

    <TranscriptList :messages="messages" :density="density" />

    <BottomInput :isStreaming="isStreaming" @send="handleSend" />
  </div>
</template>
