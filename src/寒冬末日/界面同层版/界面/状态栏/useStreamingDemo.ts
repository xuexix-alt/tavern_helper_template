...
import { bumpGeneratedImageEntityRevision, readGeneratedImageEntityRevision } from './generatedImageEntityRevision';
...
import {
  ...
  type RenderableGeneratedImageArtifact,
} from './generatedImageRenderableArtifacts';
...
import {
  buildTranscriptWindowPageOptions,
  resolveTranscriptWindowRange,
  type TranscriptWindowPageOption,
...
type RenderableGeneratedImage = RenderableGeneratedImageArtifact;
...
type ReprocessLatestAssistantOptions = {
  source?: 'manual' | 'auto_reload';
  silent?: boolean;
};
...
const TRANSCRIPT_UI_WINDOW_SIZE = 6;
const PLUGIN_NATIVE_HOST_ASSISTANT_ANCHOR_COUNT = 3;
const PLUGIN_NATIVE_HOST_SHADOW_WINDOW_MESSAGE_COUNT = 6;
...
const PLUGIN_NATIVE_RESPONSE_HYDRATION_DEDUPE_MS = 1200;
const PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_TRIGGER_GRACE_MS = 1500;
const PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_DEDUPE_MS = 60_000;
const PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_MAX_PER_RUN = 8;
const PLUGIN_NATIVE_SAME_LAYER_PROMPT_SCAN_DEDUPE_MS = 600;
const PLUGIN_NATIVE_SAME_LAYER_PROMPT_SCAN_MAX_ROOTS = 8;
const PLUGIN_NATIVE_PROMPT_HANDOFF_TTL_MS =
  IMAGE_GENERATION_LLM_HANDOFF_TIMEOUT_MS + IMAGE_GENERATION_LLM_RESPONSE_HANDOFF_GRACE_MS + 30_000;
...
const PLUGIN_NATIVE_IFRAME_PROCESSOR_MODULE_URLS = [
  '/scripts/extensions/third-party/st-chatu8/utils/iframe/index.js',
  '/scripts/extensions/third-party/st-chatu8/utils/iframe.js',
] as const;
const PLUGIN_NATIVE_IFRAME_PROCESSOR_RETRY_MS = 3000;
...
const SAME_LAYER_RENDERED_IMAGE_CONTAINER_SELECTOR = [
  ...
  `${CHATU8_IMAGE_CONTAINER_SELECTOR} img`,
  '.st-chatu8-image-container img',
  `.${FALLBACK_IMAGE_CLASSES.inline} img`,
  `.${FALLBACK_IMAGE_CLASSES.item} img`,
].join(', ');
...
type PluginImageGenerationHandoffProgress = {
  ...
  hasDomHandoff: boolean;
  complete: boolean;
};
type PluginNativePromptHandoff = {
  messageId: number;
...
function isVariableUpdateOnlyGenerationText(text: string): boolean {
  ...
  if (!new RegExp(`<${MVU_UPDATE_VARIABLE_TAG_PATTERN}\\b`, 'i').test(source)) return false;
...
function extractMvuUpdateVariableBlocks(text: string): string[] {
  ...
  const pattern = new RegExp(
    `<${MVU_UPDATE_VARIABLE_TAG_PATTERN}\\b[^>]*>[\\s\\S]*?(?:<\\/${MVU_UPDATE_VARIABLE_TAG_PATTERN}>|$)`,
    'gi',
  );
...
function countMvuJsonInstructionMarkers(text: string): number {
...
function resolveHostMessageText(message: any): string {
  const messageText = String(message?.message ?? '');
  const mesText = String(message?.mes ?? '');
...
function isSameLayerTranscriptElement(element: Element | null | undefined): boolean {
...
function collectOriginalHostMessageRoots(message_id: number): HTMLElement[] {
  const roots: HTMLElement[] = [];
  const pushRoot = (root: HTMLElement | null | undefined) => {
    if (!root) return;
    if (isSameLayerTranscriptElement(root)) return;
    if (!root.matches?.('.mes') && !root.closest?.('#chat')) return;
    if (roots.includes(root)) return;
    roots.push(root);
  };

  const mesid = Math.trunc(Number(message_id));
  if (!Number.isFinite(mesid) || mesid < 0) return roots;
  const selectors = [
  ...
  const docs = collectReachableHostDocuments();
  const orderedDocs = [...docs.filter(doc => doc !== document), ...docs.filter(doc => doc === document)];
  for (const doc of orderedDocs) {
    for (const selector of selectors) {
      const candidate = doc.querySelector(selector) as HTMLElement | null;
      pushRoot((candidate?.closest?.('.mes') as HTMLElement | null) ?? candidate);
    }
  }

  try {
    if (typeof retrieveDisplayedMessage === 'function') {
      const el = retrieveDisplayedMessage(mesid)?.get?.(0) as HTMLElement | undefined;
      const mes = el?.closest?.('.mes') as HTMLElement | null | undefined;
      pushRoot(mes ?? el);
    }
  } catch {
    // ignore
...
function readHostMesTextRenderedHtmlFromRoots(message_id: number): string {
  for (const root of collectOriginalHostMessageRoots(message_id)) {
    const mesText =
...
function readHostRenderedMessageHtml(message_id: number): string {
  ...
  try {
    ...
    const normalizedHtml = normalizeDisplayedHtml(html);
    const markerHtml = preserveChatu8PromptTokenPlacementMarkersHtml(normalizedHtml);
    return stripVisibleChatu8PromptTokensHtml(markerHtml);
  } catch {
    return '';
  }
}
...
function buildFinalHtml(
  ...
  html = preserveChatu8PromptTokenPlacementMarkersHtml(html);
...
function hydratePluginNativePromptPlaceholdersHtml(
  html: string,
  roots: HTMLElement[],
  placementHints: NativeFirstImageArtifact[] = [],
): string {
  const source = String(html ?? '');
  const validRoots = Array.isArray(roots) ? roots.filter(Boolean) : [];
  if (!source.trim() || validRoots.length === 0) return source;
  if (
    /\b(?:st-chatu8-image-button|image-tag-button|st-chatu8-image-span|image-tag-placeholder|assistant-fallback-inline-image|assistant-fallback-generated-image)\b/.test(
      source,
    )
  ) {
    return source;
  ...
  const hintByTokenCompare = new Map<string, NativeFirstImageArtifact>();
  for (const hint of placementHints) {
    const tokenCompare = normalizePromptTokenForCompare(String(hint.promptToken ?? ''));
    if (!tokenCompare || hintByTokenCompare.has(tokenCompare)) continue;
    hintByTokenCompare.set(tokenCompare, hint);
  }

  const placeholders: Array<{ html: string; tokenCompare: string; anchorText: string }> = [];
  const seen = new Set<string>();
  for (const root of validRoots) {
    const nodes = Array.from(root.querySelectorAll(`${CHATU8_IMAGE_SPAN_SELECTOR}, ${CHATU8_IMAGE_BUTTON_SELECTOR}`));
    for (const node of nodes) {
      const element = node as HTMLElement;
      const tokenCompare = normalizePromptTokenForCompare(
        extractPromptToken(
          [
            element.getAttribute('data-image-tag') ?? '',
            element.getAttribute('data-link') ?? '',
            element.getAttribute('data-prompt-token') ?? '',
            element.textContent ?? '',
            element.outerHTML,
          ].join('\n'),
        ),
      ...
      const fallbackKey = fallbackKeyParts.some(Boolean) ? [element.tagName, ...fallbackKeyParts].join('|') : '';
      const key = tokenCompare || fallbackKey || element.outerHTML;
      if (seen.has(key)) continue;
      seen.add(key);
      const anchorText = String(hintByTokenCompare.get(tokenCompare)?.anchorText ?? '').trim();
      placeholders.push({ html: element.outerHTML, tokenCompare, anchorText });
    }
  }
  ...
  for (const placeholder of placeholders) {
    if (!placeholder.tokenCompare) continue;
    const nativePromptTarget = takeNativePromptTokenPlacementTarget(nativePromptTokenTargets, placeholder.tokenCompare);
    if (!nativePromptTarget) continue;
    const template = doc.createElement('template');
    template.innerHTML = placeholder.html;
    const replacement = template.content.firstElementChild;
    if (!replacement) continue;
    nativePromptTarget.replaceWith(replacement);
    hintByTokenCompare.delete(placeholder.tokenCompare);
    placedCount += 1;
  }

  for (const placeholder of placeholders) {
    if (!placeholder.anchorText) continue;
    if (!placeholder.tokenCompare || !hintByTokenCompare.has(placeholder.tokenCompare)) continue;
    const anchor = resolveInlineAnchorTarget(doc.body, placeholder.anchorText);
    if (!anchor) continue;
    const template = doc.createElement('template');
    template.innerHTML = placeholder.html;
    ...
    reference.after(replacement);
    insertionRefs.set(anchor, replacement);
    hintByTokenCompare.delete(placeholder.tokenCompare);
    placedCount += 1;
  }

  if (placedCount > 0 || nativePromptTokenTargets.length > 0) {
  ...
  container.innerHTML = placeholders.map(placeholder => placeholder.html).join('');
...
function readChatu8ExtraImageRecords(messageId: number): Record<string, any>[] {
  const message = readChatMessageDetail(messageId);
...
function countNativeFirstArtifactSources(
  ...
  for (const artifact of artifacts) {
    counts[artifact.source] += 1;
  }
  return counts;
}

type GalleryGeneratedImageRefCacheEntry = {
  signature: string;
...
function summarizeGalleryRefCacheValue(value: unknown): string {
  if (value == null) return '';
  if (typeof value === 'number' || typeof value === 'boolean') return String(value);
  const source = String(value ?? '').trim();
  if (!source) return '';
  if (source.length <= 96) return source;
  return `${source.length}:${source.slice(0, 48)}:${source.slice(-24)}`;
}

function summarizeGalleryRefCacheObject(input: unknown): string {
  if (!input || typeof input !== 'object') return summarizeGalleryRefCacheValue(input);
  ...
  return [
    ...
    .map(summarizeGalleryRefCacheValue)
    .filter(Boolean)
    .join('~');
}
...
function buildGalleryGeneratedImageRefCacheSignature(input: {
  ...
  promptTokens: string[];
  hostDomArtifacts: RenderableGeneratedImage[];
  extraRecords: Record<string, any>[];
  cacheEntries: unknown[];
  mesTagEntries: unknown[];
}): string {
  return [
    `message:${input.messageId}`,
    `revision:${readGeneratedImageEntityRevision(input.messageId)}`,
    `created:${input.createdOrderBase}`,
    `raw:${buildDebugMessageSignature(input.rawMessage)}`,
    `tokens:${summarizeGalleryRefCacheArray(input.promptTokens)}`,
    `host:${summarizeGalleryRefCacheArray(input.hostDomArtifacts)}`,
    `extra:${summarizeGalleryRefCacheArray(input.extraRecords)}`,
    `cache:${summarizeGalleryRefCacheArray(input.cacheEntries)}`,
    `mes:${summarizeGalleryRefCacheArray(input.mesTagEntries)}`,
  ].join('\n');
}
...
function readCachedGeneratedImageRefsForMessage(input: {
  ...
  if (cached.refs.length === 0 && Date.now() - cached.cachedAt > GALLERY_REF_EMPTY_CACHE_TTL_MS) {
    galleryGeneratedImageRefCache.delete(input.messageId);
    return null;
  }
  return cached.refs;
...
function writeCachedGeneratedImageRefsForMessage(input: {
  ...
  galleryGeneratedImageRefCache.delete(input.messageId);
  galleryGeneratedImageRefCache.set(input.messageId, {
    signature: input.signature,
    refs: input.refs,
    cachedAt: Date.now(),
  });
  while (galleryGeneratedImageRefCache.size > GALLERY_REF_CACHE_MAX_ENTRIES) {
    const oldestKey = galleryGeneratedImageRefCache.keys().next().value;
    if (oldestKey == null) break;
    galleryGeneratedImageRefCache.delete(oldestKey);
  }
}

function clearGeneratedImageRefCacheForMessageIds(messageIds: number[]) {
  for (const id of messageIds) {
    const normalizedId = Math.trunc(Number(id));
    if (Number.isFinite(normalizedId) && normalizedId >= 0) {
      galleryGeneratedImageRefCache.delete(normalizedId);
    }
  }
}

function clearGeneratedImageRefCache() {
  galleryGeneratedImageRefCache.clear();
}
...
function isPluginNativeRenderableImage(image: RenderableGeneratedImage): boolean {
  const source = String((image as Partial<NativeFirstRenderableGeneratedImage>)?.source ?? '');
  return source === 'host_dom' || source === 'extra' || source === 'mes_tag';
}
...
function appendChatu8ArtifactsToHtml(html: string, renderSource: string, messageId: number): string {
  ...
  const originalHostRoots = collectOriginalHostMessageRoots(messageId);
  const nativeFirstArtifacts = readNativeFirstArtifactsForMessage({
    messageId,
    rawMessage: renderSource,
    hostDomArtifacts: extractRenderedImagesFromRoots(messageId),
  });
  const nativeFirstImages = nativeFirstArtifacts
    .map(toRenderableImageFromNativeArtifact)
    .filter((image): image is NativeFirstRenderableGeneratedImage => image !== null);
  const sourceCounts = countNativeFirstArtifactSources(nativeFirstArtifacts);
  ...
  const hasPluginNativeArtifacts = countPluginNativeImageArtifacts(originalHostRoots) > 0;
  const hostPluginNativeArtifactCount = countPluginNativeImageArtifacts(originalHostRoots);

  const renderMode = chooseImageRenderMode({
    hasPluginNativeDom: hasPluginNativeArtifacts,
    hostPluginNativeDomArtifactCount: hostPluginNativeArtifactCount,
  ...
  const suppressedDataBackfill =
    renderMode === 'plugin-native' && renderableDataBackfillCandidateCount > hostPluginNativeArtifactCount;
  const partialNativeDomSuspected =
    hasPluginNativeArtifacts && pluginNativeImages.length > hostPluginNativeArtifactCount;
  const hydratedHtml = hydratePluginNativePromptPlaceholdersHtml(
    htmlWithImages,
    originalHostRoots.length > 0
      ? originalHostRoots
      : iframeRoots.length > 0
        ? iframeRoots
        : resolveDisplayedMessageRoots(messageId),
    nativeFirstArtifacts,
  );
  recordComponentDebugTrace({
    scope: 'inlineImageHydration',
    event: 'append_artifacts',
    payload: {
      messageId,
      variant: 'transcript-inline',
      rawLength: String(renderSource ?? '').length,
      htmlLengthBefore: String(html ?? '').length,
      htmlLengthAfter: hydratedHtml.length,
      promptTokenCount: promptTokens.length,
      iframeRootCount: iframeRoots.length,
      originalHostRootCount: originalHostRoots.length,
      existingSourceCount: existingSources.size,
      hostPluginNativeArtifactCount,
      hostDomCount: sourceCounts.host_dom,
      extraCount: sourceCounts.extra,
      mesTagCount: sourceCounts.mes_tag,
      cacheCount: sourceCounts.cache,
      dataBackfillCandidateCount,
      renderableDataBackfillCandidateCount,
      nativeFirstCount: nativeFirstArtifacts.length,
      nativeRenderableCount: nativeFirstImages.length,
      pluginNativeCount: pluginNativeImages.length,
      compatibilityCount: compatibilityImages.length,
      dedupedInjectCount: dedupedImages.length,
      suppressedDataBackfill,
      partialNativeDomSuspected,
      renderMode,
    },
  });
  return hydratedHtml;
  ...
  try {
    ...
    if (hostDoc) {
      for (const selector of selectors) {
        ...
        if (el) pushRoot((el.closest?.('.mes') as HTMLElement | null) ?? el);
      }
    }
  } catch {
    // 跨域时忽略
...
function collectSameLayerRenderedMessageRoots(messageId: number): HTMLElement[] {
  const mesid = Math.trunc(Number(messageId));
  if (!Number.isFinite(mesid) || mesid < 0) return [];

  const selectors = [
    `.transcript-entry[data-message-id='${mesid}'] .assistant-body`,
    `.assistant-card[data-message-id='${mesid}'] .assistant-body`,
    `.assistant-body[data-message-id='${mesid}']`,
  ];
  const roots: HTMLElement[] = [];
  for (const selector of selectors) {
    for (const root of Array.from(document.querySelectorAll(selector)) as HTMLElement[]) {
      if (!isSameLayerTranscriptElement(root)) continue;
      if (root.closest('.gallery-panel, .assistant-gallery-image')) continue;
...
let pluginNativeIframeProcessorModulePromise: Promise<PluginNativeIframeProcessorModule | null> | null = null;
let pluginNativeIframeProcessorModuleValue: PluginNativeIframeProcessorModule | null = null;
let pluginNativeIframeProcessorModuleFailedAt = 0;

function isPluginNativeIframeProcessorModule(value: unknown): value is PluginNativeIframeProcessorModule {
...
async function loadPluginNativeIframeProcessorModule(): Promise<PluginNativeIframeProcessorModule | null> {
  if (pluginNativeIframeProcessorModuleValue) return pluginNativeIframeProcessorModuleValue;
  if (
    pluginNativeIframeProcessorModuleFailedAt > 0 &&
    Date.now() - pluginNativeIframeProcessorModuleFailedAt < PLUGIN_NATIVE_IFRAME_PROCESSOR_RETRY_MS
  ) {
    return null;
  }
  if (!pluginNativeIframeProcessorModulePromise) {
    pluginNativeIframeProcessorModulePromise = (async () => {
      const failures: { url: string; error: string }[] = [];
      for (const url of PLUGIN_NATIVE_IFRAME_PROCESSOR_MODULE_URLS) {
        try {
          const module = await import(/* @vite-ignore */ url);
          if (isPluginNativeIframeProcessorModule(module)) {
            pluginNativeIframeProcessorModuleValue = module;
            pluginNativeIframeProcessorModuleFailedAt = 0;
            return module;
          }
          failures.push({ url, error: 'module did not expose a st-chatu8 iframe processor export' });
        } catch (error) {
          failures.push({
            url,
      ...
      pluginNativeIframeProcessorModuleFailedAt = Date.now();
      console.warn('[stream-demo] failed to load st-chatu8 iframe processor module', {
        urls: [...PLUGIN_NATIVE_IFRAME_PROCESSOR_MODULE_URLS],
        failures,
      });
      return null;
    })()
  ...
  return pluginNativeIframeProcessorModulePromise;
}
...
function collectUnclaimedSameLayerChatu8PromptRoots(messageIds: number[] = []): HTMLElement[] {
  ...
  const selectors =
    normalizedMessageIds.size > 0
      ? [...normalizedMessageIds]
          .flatMap(id => [
            `.assistant-body.mes_text[data-message-id='${id}']`,
            `.transcript-entry[data-message-id='${id}'] .assistant-body.mes_text`,
          ])
          .join(', ')
      : '.assistant-body.mes_text[data-message-id]';

  if (!selectors) return roots;

  for (const root of Array.from(document.querySelectorAll(selectors)) as HTMLElement[]) {
    const scanRoot = resolvePluginNativeSameLayerPromptScanRoot(root);
    if (seen.has(scanRoot)) continue;
    if (!isSameLayerTranscriptElement(root)) continue;
    if (root.closest('.gallery-panel, .assistant-gallery-image')) continue;
    if (!/image###/i.test(root.textContent ?? '')) continue;
    if (hasNativePluginPromptControls(root)) continue;

    seen.add(scanRoot);
    roots.push(scanRoot);
    if (roots.length >= PLUGIN_NATIVE_SAME_LAYER_PROMPT_SCAN_MAX_ROOTS) break;
  }

  return roots;
}

function resolveSameLayerRenderedImageCarrier(image: HTMLImageElement): HTMLElement | null {
  const placeholderCarrier = image.closest(CHATU8_IMAGE_SPAN_SELECTOR) as HTMLElement | null;
  if (placeholderCarrier) return placeholderCarrier;

  return image.closest(
    `${CHATU8_IMAGE_CONTAINER_SELECTOR}, .st-chatu8-image-container, .assistant-fallback-inline-image, .assistant-fallback-generated-image`,
  ) as HTMLElement | null;
}

function resolveSameLayerPromptCarrierForImage(image: HTMLImageElement, root: HTMLElement): HTMLElement | null {
  const carrier = resolveSameLayerRenderedImageCarrier(image);
  ...
  const hasPromptPayload = (element: Element | null): element is HTMLElement => {
    ...
    if (!element.matches?.(CHATU8_IMAGE_BUTTON_SELECTOR)) return false;
    return Boolean(
      String(
  ...
  let sibling = carrier?.previousElementSibling ?? image.previousElementSibling;
  while (sibling) {
    if (hasPromptPayload(sibling)) return sibling;
    if (sibling.matches?.(CHATU8_IMAGE_SPAN_SELECTOR)) break;
    sibling = sibling.previousElementSibling;
  }

  const candidates = Array.from(root.querySelectorAll(CHATU8_IMAGE_BUTTON_SELECTOR)) as HTMLElement[];
  if (requestId) {
...
function mergeRenderableGeneratedImages(images: RenderableGeneratedImage[]): RenderableGeneratedImage[] {
  const out: RenderableGeneratedImage[] = [];
  ...
  for (const image of images) {
    ...
    if (!src) continue;
    const key = src;
    const normalizedImage: RenderableGeneratedImage = {
      ...image,
      src,
    ...
    if (existingIndex !== undefined) {
      out[existingIndex] = mergeRenderableGeneratedImageArtifact(out[existingIndex], normalizedImage);
      continue;
    }
    indexByKey.set(key, out.length);
    out.push(normalizedImage);
  }

  return out;
}

function extractSameLayerRenderedImagesFromTranscript(messageId: number): RenderableGeneratedImage[] {
  const out: RenderableGeneratedImage[] = [];
  let order = 0;
  for (const root of collectSameLayerRenderedMessageRoots(messageId)) {
    ...
    for (const image of images) {
      ...
      const carrier = resolveSameLayerRenderedImageCarrier(image);
      const promptButton = resolveSameLayerPromptCarrierForImage(image, root);
      const requestId = String(
        image.dataset.requestId ??
          carrier?.dataset.requestId ??
          carrier?.dataset.samelayerRequestId ??
          promptButton?.dataset.requestId ??
          promptButton?.dataset.samelayerRequestId ??
          image.getAttribute('data-request-id') ??
          carrier?.getAttribute('data-request-id') ??
          carrier?.getAttribute('data-samelayer-request-id') ??
          promptButton?.getAttribute('data-request-id') ??
          promptButton?.getAttribute('data-samelayer-request-id') ??
          '',
      ).trim();
      const promptToken = String(
        image.dataset.promptToken ??
          carrier?.dataset.promptToken ??
          carrier?.dataset.imageTag ??
          carrier?.dataset.link ??
          promptButton?.dataset.promptToken ??
          promptButton?.dataset.imageTag ??
          promptButton?.dataset.link ??
          image.getAttribute('data-prompt-token') ??
          carrier?.getAttribute('data-prompt-token') ??
          carrier?.getAttribute('data-image-tag') ??
          carrier?.getAttribute('data-link') ??
          promptButton?.getAttribute('data-prompt-token') ??
          promptButton?.getAttribute('data-image-tag') ??
          promptButton?.getAttribute('data-link') ??
          '',
      ).trim();
      out.push({
        ...
        title: `楼层 #${messageId} · 图 ${order + 1}`,
      });
      order += 1;
    }
  }

  return mergeRenderableGeneratedImages(out);
}

function collectRenderableImagesForGalleryMessage(
  messageId: number,
  hostDomArtifacts?: RenderableGeneratedImage[],
): RenderableGeneratedImage[] {
  return mergeRenderableGeneratedImages([
    ...(hostDomArtifacts ?? extractRenderedImagesFromRoots(messageId)),
    ...extractSameLayerRenderedImagesFromTranscript(messageId),
  ]);
}
...
function extractRenderedImagesFromRoots(messageId: number): RenderableGeneratedImage[] {
  ...
  const pushImage = (
    ...
    const image: RenderableGeneratedImage = {
      markerId: undefined,
      src,
      alt: String(altLike ?? 'generated image').trim() || 'generated image',
      promptToken: String(promptLike ?? '').trim() || undefined,
  ...
  for (const root of collectOriginalHostMessageRoots(messageId)) {
    allRoots.add(root);
  }
  if (allRoots.size === 0) {
  ...
  const searchInDocument = (doc: Document) => {
    ...
    for (const selector of displayedSelectors) {
      for (const el of Array.from(doc.querySelectorAll(selector)) as HTMLElement[]) {
        if (isSameLayerTranscriptElement(el)) continue;
        allRoots.add(el);
      }
    }
  };
...
function buildGeneratedImageRefsForMessage(input: {
  ...
  const extraRecords = readChatu8ExtraImageRecords(messageId);
  const cacheEntries = readChatu8CacheEntries(messageId);
  const mesTagEntries = parseNativeMesImageTags({
    messageId,
    rawMessage: input.rawMessage,
  });
  const cacheSignature = buildGalleryGeneratedImageRefCacheSignature({
    messageId,
    rawMessage: input.rawMessage,
    createdOrderBase,
    promptTokens,
  ...
  if (cachedRefs) {
    recordComponentDebugTrace({
      scope: 'imageSourceResolver',
      event: 'build_refs',
      payload: {
        messageId,
        variant: 'cache-hit',
        rawLength: String(input.rawMessage ?? '').length,
        promptTokenCount: promptTokens.length,
        hostDomInputCount: hostDomArtifacts.length,
        extraRecordCount: extraRecords.length,
        mesTagEntryCount: mesTagEntries.length,
        cacheEntryCount: cacheEntries.length,
        refCount: cachedRefs.length,
      },
    });
    return cachedRefs;
  }
  ...
  const sourceCounts = countNativeFirstArtifactSources(
    readNativeFirstArtifactsForMessage({
      messageId,
      rawMessage: input.rawMessage,
      hostDomArtifacts,
    }),
  ...
  if (readyEntities.length === 0 && hostDomArtifacts.length > 0) {
    const fallbackRefs = hostDomArtifacts
    ...
    recordComponentDebugTrace({
      ...
      payload: {
        ...
        mesTagCount: sourceCounts.mes_tag,
        cacheCount: sourceCounts.cache,
        nativeRenderableCount: nativeRenderableImages.length,
  ...
  const refs = readyEntities.map((entity, index) => {
...
function readMutationMessageIdFromElement(element: Element | null): number | null {
  ...
  const directMessageId =
    ...
    (element as HTMLElement).dataset?.messageIndex ??
  ...
  const dataMessageCarrier = element.closest?.('[data-message-id], [data-message-index]') as HTMLElement | null;
  const normalizedDataMessageId = normalizeMutationMessageId(
    dataMessageCarrier?.dataset?.messageId ??
      dataMessageCarrier?.getAttribute('data-message-id') ??
      dataMessageCarrier?.dataset?.messageIndex ??
      dataMessageCarrier?.getAttribute('data-message-index') ??
      '',
  );
...
export function useStreamingDemo() {
  ...
  let autoMvuReloadReprocessTimer = 0;
  let autoMvuReloadReprocessIdleHandle = 0;
  const hostImageDataReconcileTimers = new Set<number>();
  const galleryInitialCacheTimers = new Set<number>();
  const pendingGeneratedImageRefreshMessageIds = new Set<number>();
  const autoMvuReloadReprocessMessageIds = new Set<number>();
  const pluginNativeResponseHydrationSessions = new Set<string>();
  const pluginNativeResponseHydrationStartedAt = new Map<string, number>();
  const pluginNativeSameLayerPromptScanStartedAt = new Map<string, number>();
  ...
  const pluginNativePromptButtonTriggerAt = new Map<string, number>();
  ...
  function normalizePluginNativeHandoffMessageId(value: unknown): number | null {
  ...
  function collectRecentAssistantMessageIdsForPluginNativeHandoff(): number[] {
    ...
    transcript.value
      ...
      .slice(0, PLUGIN_NATIVE_HOST_ASSISTANT_ANCHOR_COUNT)
      .forEach(remember);
  ...
  function collectPluginNativeHandoffMessageIds(payload: unknown = null): number[] {
    ...
    const visit = (value: unknown, depth = 0) => {
      ...
      for (const key of ['messageId', 'message_id', 'mesId', 'mesid', 'id']) {
        remember(record[key]);
      }
      for (const key of ['message', 'detail', 'payload', 'data', 'context']) {
  ...
  function schedulePluginNativeHostRenderHandoff(reason: string, payload: unknown = null) {
    const messageIds = collectPluginNativeHandoffMessageIds(payload);
    if (messageIds.length === 0) return;

    recordLifecycleTrace('pluginNativeHostRenderHandoff', 'scheduled', {
      reason,
      messageIds,
    });
    ...
    for (const delayMs of [80, 240] as const) {
      const timer = window.setTimeout(() => {
        hostImageDataReconcileTimers.delete(timer);
        syncPendingRequestHintsFromDom();
        syncTranscriptItemsFromHostData(`${reason}:host_render_delay_${delayMs}`, messageIds);
        queueGeneratedImageEntityRefresh(messageIds, `${reason}:host_render_delay_${delayMs}`);
      }, delayMs);
  ...
  function markPluginNativeLlmImageGenerationFinished(payload: unknown) {
    ...
    schedulePluginNativeHostRenderHandoff('plugin_native_llm_image_generation_response', payload);
  }
  ...
  function beginPluginNativePromptHandoff(messageId: number, source: 'transcript' | 'gallery' = 'transcript') {
    const normalizedId = Math.trunc(Number(messageId));
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return;
    activePluginNativePromptHandoff = {
      messageId: normalizedId,
      source,
      startedAt: Date.now(),
      token: createTraceId('plugin-prompt-handoff'),
    };
    recordLifecycleTrace('pluginNativePromptHandoff', 'begin', {
      messageId: normalizedId,
      source,
      token: activePluginNativePromptHandoff.token,
    });
  }

  function readActivePluginNativePromptHandoff(): PluginNativePromptHandoff | null {
    const handoff = activePluginNativePromptHandoff;
    if (!handoff) return null;
    const ageMs = Date.now() - handoff.startedAt;
    if (ageMs <= PLUGIN_NATIVE_PROMPT_HANDOFF_TTL_MS) return handoff;
    activePluginNativePromptHandoff = null;
    recordLifecycleTrace('pluginNativePromptHandoff', 'expired', {
      messageId: handoff.messageId,
      source: handoff.source,
      ageMs,
      ttlMs: PLUGIN_NATIVE_PROMPT_HANDOFF_TTL_MS,
      token: handoff.token,
    });
    return null;
  }

  function hasActivePluginNativePromptHandoff(messageId: number): boolean {
    const normalizedId = Math.trunc(Number(messageId));
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return false;
    const handoff = readActivePluginNativePromptHandoff();
    return handoff?.source === 'transcript' && handoff.messageId === normalizedId;
  }
  ...
  function finishPluginNativePromptHandoff(messageId: number, reason: string) {
    ...
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return;
    const handoff = readActivePluginNativePromptHandoff();
    if (!handoff || handoff.messageId !== normalizedId) return;
    activePluginNativePromptHandoff = null;
    recordLifecycleTrace('pluginNativePromptHandoff', 'finish', {
      reason,
      messageId: normalizedId,
      source: handoff.source,
      ageMs: Date.now() - handoff.startedAt,
      token: handoff.token,
    });
  }
  ...
  function bindPluginNativeLlmImageGenerationEvents(): StopHandle[] {
    ...
    const stops: StopHandle[] = [
      ...
      eventOn(CHATU8_REGEX_TEST_MESSAGE_EVENT as any, (...payload: unknown[]) => {
        schedulePluginNativeHostRenderHandoff('plugin_native_regex_test_message', payload);
      }),
      eventOn(CHATU8_REGEX_RESULT_MESSAGE_EVENT as any, (...payload: unknown[]) => {
        schedulePluginNativeHostRenderHandoff('plugin_native_regex_result_message', payload);
      }),
      ...CHATU8_AUTO_CLICK_COMPLETE_EVENTS.map(eventName =>
        eventOn(eventName as any, (...payload: unknown[]) => {
          schedulePluginNativeHostRenderHandoff('plugin_native_auto_click_complete', payload);
        }),
      ),
    ];
    if (typeof tavern_events !== 'undefined' && tavern_events?.CHARACTER_MESSAGE_RENDERED) {
      stops.push(
        eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED, (...payload: unknown[]) => {
          schedulePluginNativeHostRenderHandoff('plugin_native_character_message_rendered', payload);
        }),
      );
    }
    return stops.filter(Boolean) as StopHandle[];
  }
    ...
    for (const doc of collectReachableHostDocuments()) {
      ...
      for (const button of buttons) {
        ...
        const carrierMessageId =
          normalizeMutationMessageId(carrier?.dataset?.messageId) ??
          normalizeMutationMessageId(carrier?.getAttribute?.('data-message-id')) ??
          normalizeMutationMessageId(carrier?.dataset?.messageIndex) ??
          normalizeMutationMessageId(carrier?.getAttribute?.('data-message-index')) ??
          normalizeMutationMessageId(carrier?.getAttribute?.('mesid'));
        const recentIntent = imageRecentIntentStore.read();
  ...
  function buildHostImageDataSignature(message: any): string {
    const rawMessage = resolveHostMessageText(message).trim();
  ...
  function listHostImageDataCandidates(messageIds: number[] = []): any[] {
    ...
    const isCandidateMessage = (message: any) => {
    ...
    if (normalizedIds.size > 0) {
      return [...normalizedIds].map(messageId => readHostChatMessageById(messageId)).filter(isCandidateMessage);
    }

    return listAllChatMessages().filter(isCandidateMessage);
  }
  ...
  function scheduleHostImageDataReconcile(reason: string, messageIds: number[] = []) {
    ...
    for (const delayMs of HOST_IMAGE_RESPONSE_RECONCILE_DELAYS_MS) {
      const timer = window.setTimeout(() => {
        ...
        logImageBridge('host-data-reconcile-probe', {
          reason,
          delayMs,
          messageIds: normalizedMessageIds,
          changedMessageIds,
        ...
        discoverRecentNativeGalleryImages(`${reason}:delay_${delayMs}`, normalizedMessageIds);
  ...
  function hasHostMessageDom(messageId: number): boolean {
    ...
    return collectHostDocuments().some(doc =>
      Boolean(doc.querySelector(`.mes[mesid="${normalizedId}"], .mes[data-message-id="${normalizedId}"]`)),
    );
  }
  ...
  /**
   * 将最近 6 楼保留为原生离屏 shadow window，供 st-chatu8 继续走酒馆原生 DOM 链；
   * 更早的宿主楼层仍设为 is_hidden，控制 DOM/插件扫描成本。
   */
    ...
    try {
      do {
        ...
        if (showPatch.length > 0) {
          ...
          await setChatMessages(showPatch, { refresh: 'affected' });
        }
        const hidePatch = metas
          .filter(item => !nativeShadowWindowIds.includes(item.message_id))
          .filter(item => item.is_hidden !== true)
          .map(item => ({ message_id: item.message_id, is_hidden: true }));
        if (hidePatch.length > 0) {
          await setChatMessages(hidePatch, { refresh: 'none' });
        }
  ...
  async function emitTavernLifecycleEventWithTimeout(
    eventType: unknown,
    args: unknown[],
    options: { eventName: string; messageId: number; traceId: string; timeoutMs?: number },
  ): Promise<boolean> {
    if (typeof eventEmit !== 'function') return false;
    const timeoutMs = options.timeoutMs ?? OFFICIAL_LIFECYCLE_EVENT_AWAIT_TIMEOUT_MS;
    let settled = false;
    let timeoutId: number | null = null;
    const emitPromise = Promise.resolve()
      .then(() => eventEmit(eventType as any, ...(args as any[])))
      .then(() => {
        settled = true;
        return true;
      })
    ...
    const timeoutPromise = new Promise<false>(resolve => {
      timeoutId = window.setTimeout(() => {
        if (!settled) {
          recordLifecycleTrace(
            'emitOfficialGenerationLifecycle',
            `${options.eventName}_await_timeout`,
            {
              messageId: options.messageId,
              timeoutMs,
            },
  ...
  function readHostChatMessageById(messageId: number): any | null {
    const normalizedId = Math.trunc(Number(messageId));
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return null;
    const detail = readChatMessageDetail(normalizedId);
    if (!detail || typeof detail !== 'object') return null;
    return (
      normalizeHostChatMessages([
        {
          ...detail,
          message_id: normalizedId,
        },
  ...
  function isMessageIdInTranscriptWindow(messageId: number, range: TranscriptWindowRange | null): boolean {
    return range != null && messageId >= range.startId && messageId <= range.endId;
  }
  ...
  function refreshTranscriptItemsByIds(messageIds: number[], reason = 'manual') {
    ...
    const containerId = getActiveContainerMessageId();
    const currentWindowRange = transcriptWindowRange.value;
    ...
    for (const messageId of normalizedMessageIds) {
      ...
      if (!existingItem && !isMessageIdInTranscriptWindow(messageId, currentWindowRange)) continue;

      const hostMessage = readHostChatMessageById(messageId);
  ...
  function collectNativeShadowWindowMessageIds(anchorMessageId?: number | null): number[] {
    const normalizedAnchor = anchorMessageId == null ? null : Math.trunc(Number(anchorMessageId));
    const metas = readMessageMetasAfterContainer()
      .map(item => Math.trunc(Number(item.message_id)))
      .filter(id => Number.isFinite(id) && id >= 0)
      .sort((a, b) => b - a);
    const boundedMetas =
      normalizedAnchor != null && Number.isFinite(normalizedAnchor) && normalizedAnchor >= 0
        ? metas.filter(id => id <= normalizedAnchor)
        : metas;
    const windowIds = boundedMetas.slice(0, PLUGIN_NATIVE_HOST_SHADOW_WINDOW_MESSAGE_COUNT);
    if (
      normalizedAnchor != null &&
      Number.isFinite(normalizedAnchor) &&
      normalizedAnchor >= 0 &&
      !windowIds.includes(normalizedAnchor)
    ) {
      windowIds.unshift(normalizedAnchor);
    }
    return [...new Set(windowIds)].sort((a, b) => a - b);
  }

  function collectPluginNativeHostWindowMessageIds(messageId: number): number[] {
    return collectNativeShadowWindowMessageIds(messageId);
  }
  ...
  function persistHideStateNow(reason: string) {
    ...
    const nativeShadowWindowIds = collectNativeShadowWindowMessageIds();
    const hiddenIds = readMessageMetasAfterContainer()
      .filter(item => !nativeShadowWindowIds.includes(item.message_id))
      .map(item => item.message_id);
  ...
  function getPluginNativePromptButtonMessageRoots(messageId: number): HTMLElement[] {
    const normalizedId = Math.trunc(Number(messageId));
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return [];

    const roots: HTMLElement[] = [];
    const seen = new Set<HTMLElement>();
    const push = (root: HTMLElement | null) => {
    ...
    for (const doc of collectHostDocuments()) {
      push(doc.querySelector(`.mes[mesid="${normalizedId}"] .mes_text`) as HTMLElement | null);
      push(doc.querySelector(`.mes[data-message-index="${normalizedId}"] .mes_text`) as HTMLElement | null);
      push(doc.querySelector(`.mes_text[data-message-index="${normalizedId}"]`) as HTMLElement | null);
    }

    return roots;
  }

  function isReadyGeneratedImageElement(image: HTMLImageElement | null): boolean {
    const src = normalizeImageSrcForCompare(image?.getAttribute('src') ?? image?.currentSrc ?? '');
    return Boolean(src && (src.startsWith('data:image') || src.startsWith('blob:')));
  }

  function hasReadyImageNearPluginNativeButton(button: HTMLElement): boolean {
    let sibling = button.nextElementSibling;
    while (sibling) {
      if (sibling.matches?.(CHATU8_IMAGE_BUTTON_SELECTOR)) return false;
      if (sibling.tagName === 'IMG' && isReadyGeneratedImageElement(sibling as HTMLImageElement)) return true;
      const nestedImage = sibling.querySelector?.('img') as HTMLImageElement | null;
      if (isReadyGeneratedImageElement(nestedImage)) return true;
      sibling = sibling.nextElementSibling;
    }
    return false;
  }

  function buildPluginNativePromptButtonTriggerKey(button: HTMLElement, messageId: number, index: number): string {
    const requestId = String(
      button.dataset.requestId ??
        button.getAttribute('data-request-id') ??
        button.dataset.stableId ??
    ...
    if (requestId) return `${messageId}:request:${requestId}`;

    const prompt = String(button.getAttribute('data-image-tag') ?? button.getAttribute('data-link') ?? '').trim();
    if (prompt) return `${messageId}:prompt:${prompt.slice(0, 180)}`;

    return `${messageId}:index:${index}`;
  }

  function prunePluginNativePromptButtonTriggerKeys(now = Date.now()) {
    for (const [key, triggeredAt] of pluginNativePromptButtonTriggerAt.entries()) {
      if (now - triggeredAt > PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_DEDUPE_MS) {
        pluginNativePromptButtonTriggerAt.delete(key);
      }
    }
  }

  function collectPendingPluginNativePromptButtonsForMessage(messageId: number): HTMLElement[] {
    const buttons: HTMLElement[] = [];
    const seen = new Set<HTMLElement>();
    for (const root of getPluginNativePromptButtonMessageRoots(messageId)) {
      for (const button of Array.from(root.querySelectorAll(CHATU8_IMAGE_BUTTON_SELECTOR)) as HTMLElement[]) {
        if (seen.has(button)) continue;
        seen.add(button);
        if (button.hasAttribute('disabled') || button.getAttribute('aria-disabled') === 'true') continue;
        if (hasReadyImageNearPluginNativeButton(button)) continue;
        buttons.push(button);
      }
    }
    return buttons;
  ...
  function shouldTriggerPendingPluginNativePromptButtons(
    ...
    if (!hasActivePluginNativePromptHandoff(messageId)) return false;
  ...
  function triggerPendingPluginNativePromptButtons(
    ...
    const buttons = collectPendingPluginNativePromptButtonsForMessage(messageId);
    if (buttons.length === 0) return 0;

    let triggeredCount = 0;
    for (const [index, button] of buttons.entries()) {
      if (triggeredCount >= PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_MAX_PER_RUN) break;
      const key = buildPluginNativePromptButtonTriggerKey(button, messageId, index);
      const lastTriggeredAt = pluginNativePromptButtonTriggerAt.get(key) ?? 0;
      if (now - lastTriggeredAt <= PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_DEDUPE_MS) continue;

      pluginNativePromptButtonTriggerAt.set(key, now);
      const delayMs = triggeredCount * 120;
      window.setTimeout(() => {
        try {
          button.click();
          syncPendingRequestHintsFromDom();
        } catch (error) {
          console.warn('[stream-demo] plugin prompt button fallback click failed', {
            reason,
            messageId,
            error: error instanceof Error ? error.message : String(error),
          });
    ...
    if (triggeredCount > 0) {
      recordLifecycleTrace('imageGenerationHandoff', 'prompt_button_fallback_triggered', {
        reason,
        messageId,
        triggeredCount,
  ...
  function collectPluginImageGenerationHandoffProgress(messageId: number): PluginImageGenerationHandoffProgress {
    const normalizedId = Math.trunc(Number(messageId));
    if (!Number.isFinite(normalizedId) || normalizedId < 0) {
      return {
        requestCount: 0,
    ...
    let promptPlaceholderCount = 0;
    let readyImageCount = 0;
    let hasDomHandoff = false;

    for (const mesText of getPluginNativePromptButtonMessageRoots(normalizedId)) {
    ...
    const expectedRequestCount = Math.max(promptButtonCount, promptPlaceholderCount);
    const complete =
      requestCount > 0 &&
  ...
  function hasPluginImageGenerationHandoff(messageId: number): boolean {
    return collectPluginImageGenerationHandoffProgress(messageId).complete;
  }
  ...
  async function waitForPluginImageGenerationHandoff(messageId: number): Promise<boolean> {
    ...
    while (shouldContinuePluginNativeHandoffWait(shortDeadline, extendedDeadline)) {
      ...
      const handoffProgress = collectPluginImageGenerationHandoffProgress(normalizedId);
      if (handoffProgress.complete) {
      ...
      if (elapsedMs >= PLUGIN_NATIVE_PROMPT_BUTTON_FALLBACK_TRIGGER_GRACE_MS) {
        triggerPendingPluginNativePromptButtons('imageGenerationHandoff', normalizedId, handoffProgress);
      }
  ...
  async function withPluginNativeMessageLease<T>(
    ...
    const materializeLeaseIds = readMessageMetasAfterContainer()
      .filter(item => leaseMessageIds.includes(item.message_id))
      .filter(item => item.is_hidden === true || !hasHostMessageDom(item.message_id))
      .map(item => item.message_id);
    ...
    if (materializeLeaseIds.length > 0) {
      await setChatMessages(
        materializeLeaseIds.map(id => ({ message_id: id, is_hidden: false })),
        { refresh: 'affected' },
      );
    ...
    try {
      ...
      try {
        ...
        // The durable native shadow window keeps recent plugin-native floors rendered;
        // hide policy will re-hide only floors outside that window after this bridge turn.
      } finally {
  ...
  function shouldRunPluginNativePromptScan(reason: string, messageIds: number[]): boolean {
    const activeHandoff = readActivePluginNativePromptHandoff();
    if (activeHandoff?.source === 'transcript' && messageIds.includes(activeHandoff.messageId)) {
      return true;
    }

    if (shouldRunPluginNativePromptScanForHostLifecycle(reason, messageIds)) {
      return true;
    }

    recordLifecycleTrace('pluginNativePromptPlaceholderReconcile', 'prompt_scan_skipped', {
      reason,
      messageIds,
      activeHandoff,
    });
  ...
  function shouldRunPluginNativePromptScanForHostLifecycle(reason: string, messageIds: number[]): boolean {
    const normalizedReason = String(reason ?? '');
    const isHostLifecycleReason =
      normalizedReason.includes('generation_done') ||
      normalizedReason.includes('generation_ended') ||
      normalizedReason.includes('native_host_hydration') ||
      normalizedReason.includes('plugin_native_placeholder_dom_mutation') ||
      normalizedReason.includes('plugin_native_llm_image_generation_response');

    if (!isHostLifecycleReason) return false;

    return messageIds.some(messageId => {
      const detail = readChatMessageDetail(messageId);
      const text = String(detail?.mes ?? detail?.message ?? '');
      return collectChatu8PromptTokens(text).length > 0;
    });
  }

  function resolveActivePluginNativePromptScanMessageIds(messageIds: number[]): number[] {
    const activeHandoff = readActivePluginNativePromptHandoff();
    if (activeHandoff?.source !== 'transcript') return [];
    if (messageIds.length > 0 && !messageIds.includes(activeHandoff.messageId)) return [];
    return [activeHandoff.messageId];
  }
  ...
  function schedulePluginNativePromptPlaceholderReconcile(reason: string, messageIds: number[] = []) {
    ...
    const normalizedMessageIds = [
      ...new Set(
        [
          ...
          messageIds.length === 0 && activeHandoff?.source === 'transcript' ? activeHandoff.messageId : null,
    ...
    for (const delayMs of PLUGIN_NATIVE_PROMPT_PLACEHOLDER_RECONCILE_DELAYS_MS) {
      const runProbe = () => {
        const promptScanMessageIds = resolveActivePluginNativePromptScanMessageIds(normalizedMessageIds);
        const hostLifecycleScanMessageIds =
          promptScanMessageIds.length > 0 ? promptScanMessageIds : normalizedMessageIds;
        if (shouldRunPluginNativePromptScan(reason, hostLifecycleScanMessageIds)) {
          void runPluginNativeSameLayerPromptScan(reason, hostLifecycleScanMessageIds);
        }
        syncPendingRequestHintsFromDom();
        for (const messageId of normalizedMessageIds) {
  ...
  function collectPluginNativePromptBootstrapMessageIds(): number[] {
    return transcript.value
      ...
      .filter(messageId => {
        const detail = readChatMessageDetail(messageId);
        const text = String(detail?.mes ?? detail?.message ?? '');
        if (collectChatu8PromptTokens(text).length === 0) return false;
        return collectUnclaimedSameLayerChatu8PromptRoots([messageId]).length > 0;
      });
  }

  function schedulePluginNativePromptBootstrap(reason: string) {
    const messageIds = collectPluginNativePromptBootstrapMessageIds();
    if (messageIds.length === 0) return;
    recordLifecycleTrace('pluginNativePromptBootstrap', 'scheduled', {
      reason,
      messageIds,
    });
  ...
  async function nudgePluginNativePromptProcessingForActiveIntent(
    reason: string,
    messageIds: number[],
    roots: HTMLElement[] = [],
  ): Promise<void> {
    ...
    recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_start', {
      reason,
      messageIds: normalizedMessageIds,
    ...
    for (const messageId of normalizedMessageIds) {
      ...
      try {
        hostRendered = await ensureHostMesTextRendered(messageId);
      } catch (error) {
        recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_render_failed', {
          reason,
          messageId,
          error: error instanceof Error ? error.message : String(error),
        });
      ...
      try {
        await refreshHostMessageForPluginNativeImageTrigger(messageId);
      } catch (error) {
        recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_mutation_failed', {
          reason,
          messageId,
          error: error instanceof Error ? error.message : String(error),
        });
      ...
      try {
        if (typeof eventEmit === 'function') {
          const renderedEvent = tavern_events.CHARACTER_MESSAGE_RENDERED;
          await eventEmit(renderedEvent as any, messageId);
        }
      } catch (error) {
        recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_event_failed', {
          reason,
          messageId,
          error: error instanceof Error ? error.message : String(error),
        });
      ...
      recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_message', {
        reason,
        messageId,
        hostRendered,
      });
    ...
    syncPendingRequestHintsFromDom();
    syncTranscriptItemsFromHostData(`${reason}:module_missing_nudge`, normalizedMessageIds);
    queueGeneratedImageEntityRefresh(normalizedMessageIds, `${reason}:module_missing_nudge`);

    for (const delayMs of [160, 480, 1200, 2400, 4200, 8400] as const) {
      const timer = window.setTimeout(() => {
        hostImageDataReconcileTimers.delete(timer);
        syncPendingRequestHintsFromDom();
        syncTranscriptItemsFromHostData(`${reason}:module_missing_nudge_delay_${delayMs}`, normalizedMessageIds);
        queueGeneratedImageEntityRefresh(normalizedMessageIds, `${reason}:module_missing_nudge_delay_${delayMs}`);
        for (const messageId of normalizedMessageIds) {
          const handoffProgress = collectPluginImageGenerationHandoffProgress(messageId);
          triggerPendingPluginNativePromptButtons('prompt_scan_module_missing_nudge', messageId, handoffProgress);
          recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing_nudge_probe', {
            reason,
            delayMs,
            messageId,
            handoffProgress,
  ...
  async function runPluginNativeSameLayerPromptScan(reason: string, messageIds: number[] = []): Promise<void> {
    const normalizedMessageIds = [
      ...new Set(messageIds.map(id => Math.trunc(Number(id))).filter(id => Number.isFinite(id) && id >= 0)),
    ];
    if (normalizedMessageIds.length === 0) return;

    const roots = collectUnclaimedSameLayerChatu8PromptRoots(messageIds);
    if (roots.length === 0) return;

    const scanKey = normalizedMessageIds.join(',');
    const now = Date.now();
    const previousStartedAt = pluginNativeSameLayerPromptScanStartedAt.get(scanKey) ?? 0;
    if (now - previousStartedAt < PLUGIN_NATIVE_SAME_LAYER_PROMPT_SCAN_DEDUPE_MS) {
      recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'deduped', {
        reason,
        messageIds: normalizedMessageIds,
        rootCount: roots.length,
      });
    ...
    pluginNativeSameLayerPromptScanStartedAt.set(scanKey, now);
    if (pluginNativeSameLayerPromptScanStartedAt.size > 32) {
      const staleCutoff = now - PLUGIN_NATIVE_SAME_LAYER_PROMPT_SCAN_DEDUPE_MS * 8;
      for (const [key, startedAt] of pluginNativeSameLayerPromptScanStartedAt) {
        if (startedAt < staleCutoff) pluginNativeSameLayerPromptScanStartedAt.delete(key);
      }
    }

    const processor = await loadPluginNativeIframeProcessorModule();
    if (!processor) {
      recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'module_missing', {
        reason,
        messageIds: normalizedMessageIds,
        rootCount: roots.length,
      });
    ...
    try {
      if (typeof processor.processImagePlaceholdersForElement === 'function') {
      ...
      syncPendingRequestHintsFromDom();
      queueGeneratedImageEntityRefresh(normalizedMessageIds, `${reason}:same_layer_prompt_scan`);
      scheduleUiRefresh(['transcriptItems', 'gallery'], `${reason}:same_layer_prompt_scan`, normalizedMessageIds);
      recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'processed', {
        reason,
        messageIds: normalizedMessageIds,
        rootCount: roots.length,
        processedCount,
      ...
      recordLifecycleTrace('pluginNativeSameLayerPromptScan', 'failed', {
        reason,
        messageIds: normalizedMessageIds,
        rootCount: roots.length,
        processedCount,
      ...
      console.warn('[stream-demo] st-chatu8 same-layer prompt scan failed', {
        reason,
        messageIds: normalizedMessageIds,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async function hydratePluginNativeResponseMessages(reason: string, messageIds: number[]): Promise<void> {
    ...
    const hydrationIds = [
      ...new Set(normalizedMessageIds.flatMap(messageId => collectPluginNativeHostWindowMessageIds(messageId))),
    ].sort((a, b) => a - b);
    if (hydrationIds.length === 0) return;

    const sessionKey = hydrationIds.join(',');
    const now = Date.now();
    const previousStartedAt = pluginNativeResponseHydrationStartedAt.get(sessionKey) ?? 0;
    if (
      pluginNativeResponseHydrationSessions.has(sessionKey) ||
      now - previousStartedAt < PLUGIN_NATIVE_RESPONSE_HYDRATION_DEDUPE_MS
    ) {
      recordLifecycleTrace('pluginNativeResponseHydration', 'deduped', {
        reason,
        messageIds: normalizedMessageIds,
        hydrationIds,
        sessionKey,
    ...
    pluginNativeResponseHydrationSessions.add(sessionKey);
    pluginNativeResponseHydrationStartedAt.set(sessionKey, now);
    if (pluginNativeResponseHydrationStartedAt.size > 24) {
      const staleCutoff = now - PLUGIN_NATIVE_RESPONSE_HYDRATION_DEDUPE_MS * 4;
      for (const [key, startedAt] of pluginNativeResponseHydrationStartedAt) {
        if (startedAt < staleCutoff) pluginNativeResponseHydrationStartedAt.delete(key);
      }
    }
    ...
    const materializeHydrationIds = readMessageMetasAfterContainer()
      .filter(item => hydrationIds.includes(item.message_id))
      .filter(item => item.is_hidden === true || !hasHostMessageDom(item.message_id))
      .map(item => item.message_id);
    const releaseHydrationLease = hostVisualHideController.leaseMessageIdsForPluginNativeHandoff(
      hydrationIds,
      `${reason}:response_hydration`,
    );
    let releaseMaterializedLease = () => {};

    recordLifecycleTrace('pluginNativeResponseHydration', 'start', {
      reason,
      messageIds: normalizedMessageIds,
      hydrationIds,
      materializeHydrationIds,
      sessionKey,
    });

    try {
      normalizePluginNativeMessageMetadata(hydrationIds);
      if (materializeHydrationIds.length > 0) {
        await setChatMessages(
          materializeHydrationIds.map(id => ({ message_id: id, is_hidden: false })),
          { refresh: 'affected' },
        );
        releaseMaterializedLease = hostVisualHideController.leaseMessageIdsForPluginNativeHandoff(
          hydrationIds,
          `${reason}:response_hydration_materialized`,
        );
      }
      ...
      for (const delayMs of [120, 360, 900, 1800, 3600] as const) {
        ...
        discoverRecentNativeGalleryImages(`${reason}:response_hydration_delay_${delayMs}`, normalizedMessageIds);
        queueGeneratedImageEntityRefresh(normalizedMessageIds, `${reason}:response_hydration_delay_${delayMs}`);
      }

      recordLifecycleTrace('pluginNativeResponseHydration', 'done', {
        reason,
        messageIds: normalizedMessageIds,
        hydrationIds,
        sessionKey,
      ...
      console.warn('[stream-demo] plugin-native response hydration failed', {
        reason,
        messageIds: normalizedMessageIds,
        error: error instanceof Error ? error.message : String(error),
      });
      ...
      releaseMaterializedLease();
      releaseHydrationLease();
      pluginNativeResponseHydrationSessions.delete(sessionKey);
      queueHidePolicy(`${reason}:response_hydration_done`);
    }
  }
  ...
  async function refreshHostMessagesForPluginNativeImageCompletion(
    ...
    recordLifecycleTrace('pluginNativeImageCompletionRefresh', 'start', {
      reason,
      messageIds: normalizedMessageIds,
    });
    ...
    try {
      ...
      syncTranscriptItemsFromHostData(`${reason}:host_dom_refresh`, normalizedMessageIds);
      scheduleUiRefresh(['transcriptItems', 'gallery'], `${reason}:host_dom_refresh`, normalizedMessageIds);
      queueGeneratedImageEntityRefresh(normalizedMessageIds, `${reason}:host_dom_refresh`);
      recordLifecycleTrace('pluginNativeImageCompletionRefresh', 'done', {
        reason,
        messageIds: normalizedMessageIds,
      });
    } catch (error) {
      recordLifecycleTrace('pluginNativeImageCompletionRefresh', 'error', {
        reason,
        messageIds: normalizedMessageIds,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  function beginPendingImageTask(messageId: number, source: 'transcript' | 'gallery' = 'transcript') {
    ...
    if (source === 'transcript') {
      beginPluginNativePromptHandoff(normalizedId, source);
    }
    ...
    await runQueuedHostMessageUpdate({
      ...
      task: async () => {
        ...
        await withPluginNativeMessageLease(
          ...
          {
            beforeRelease: async () => {
              if (!shouldWaitForPluginHandoff) {
                finishPluginNativePromptHandoff(normalizedId, 'cancelled');
                return;
              }
  ...
  function handleHostRefreshEvent(name: string, payload: unknown[] = []) {
    ...
    if (domains.length === 0) {
      recordLifecycleTrace('handleHostRefreshEvent', 'skipped_empty_domains', {
        name,
        refreshType,
      });
      return;
      ...
      for (const entry of normalizedEntries) {
        normalized.push(
          buildTranscriptItem({
            id: entry.id,
            role: entry.role,
            raw: resolveHostMessageText(entry.message),
  ...
  function countMvuObjectKeys(value: unknown): number {
  ...
  function readMessageMvuDataCompleteness(messageId: number) {
    ...
    const empty = {
      ...
      currentInitializedLorebookCount: 0,
      previousStatKeyCount: 0,
      previousInitializedLorebookCount: 0,
      updateBlockCount: 0,
      instructionMarkerCount: 0,
      missingCurrentData: false,
      reason: 'invalid_message_id',
    };
    if (!Number.isFinite(normalizedId) || normalizedId < 0) return empty;

    try {
      if (typeof Mvu === 'undefined' || typeof Mvu?.getMvuData !== 'function') {
        return { ...empty, messageId: normalizedId, reason: 'mvu_unavailable' };
      }

      const currentData = Mvu.getMvuData({ type: 'message', message_id: normalizedId });
      const currentStatKeyCount = countMvuObjectKeys(_.get(currentData, 'stat_data', null));
      const currentInitializedLorebookCount = countMvuObjectKeys(_.get(currentData, 'initialized_lorebooks', null));
      const hasCurrentData = currentStatKeyCount > 0 || currentInitializedLorebookCount > 0;

      let previousStatKeyCount = 0;
      let previousInitializedLorebookCount = 0;
      for (let offset = 1; offset <= 24; offset += 1) {
        const previousId = normalizedId - offset;
        if (previousId < 0) break;
        const previousData = Mvu.getMvuData({ type: 'message', message_id: previousId });
        previousStatKeyCount = countMvuObjectKeys(_.get(previousData, 'stat_data', null));
        previousInitializedLorebookCount = countMvuObjectKeys(_.get(previousData, 'initialized_lorebooks', null));
        if (previousStatKeyCount > 0 || previousInitializedLorebookCount > 0) break;
      }

      const messageDetail = readChatMessageDetail(normalizedId);
      const rawMessage = resolveHostMessageText(messageDetail);
      const updateBlockCount = extractMvuUpdateVariableBlocks(rawMessage).length;
      const instructionMarkerCount = updateBlockCount + countMvuJsonInstructionMarkers(rawMessage);
      const hasPreviousData = previousStatKeyCount > 0 || previousInitializedLorebookCount > 0;
      ...
      return {
        ...
        hasPreviousData,
        currentStatKeyCount,
        currentInitializedLorebookCount,
        previousStatKeyCount,
        previousInitializedLorebookCount,
        updateBlockCount,
        instructionMarkerCount,
        missingCurrentData,
        reason: missingCurrentData ? 'missing_current_data' : 'ready',
      };
    } catch (error) {
      return {
        ...empty,
  ...
  async function autoReprocessLatestAssistantVariablesIfMissing(reason = 'mounted.mvu_reload_probe') {
    const latestAssistant = latestAssistantItem.value;
    if (!latestAssistant || latestAssistant.role !== 'assistant') return;
    if (busy.value || reprocessVariablesPending.value) return;
    if (autoMvuReloadReprocessMessageIds.has(latestAssistant.message_id)) return;

    const completeness = readMessageMvuDataCompleteness(latestAssistant.message_id);
    recordLifecycleTrace('mvuReloadReprocess', 'probe', {
      reason,
      assistantMessageId: latestAssistant.message_id,
      completeness,
    });
    if (!completeness.missingCurrentData) return;

    autoMvuReloadReprocessMessageIds.add(latestAssistant.message_id);
    appendLog(
      'info',
      '自动重试额外模型解析',
      `UI 重载后检测到 assistant #${latestAssistant.message_id} 的 MVU 数据缺口，已排队走原生重试。`,
    );
  ...
  function clearAutoMvuReloadReprocessSchedule() {
    if (autoMvuReloadReprocessTimer) {
      window.clearTimeout(autoMvuReloadReprocessTimer);
      autoMvuReloadReprocessTimer = 0;
    }
    if (autoMvuReloadReprocessIdleHandle) {
      const cancelIdle = (window as any).cancelIdleCallback;
      if (typeof cancelIdle === 'function') {
        cancelIdle(autoMvuReloadReprocessIdleHandle);
      }
      autoMvuReloadReprocessIdleHandle = 0;
    }
  }

  function scheduleMvuReloadReprocessIfMissing(reason = 'mounted.mvu_reload_probe') {
    clearAutoMvuReloadReprocessSchedule();
    const runIdleProbe = () => {
      const requestIdle = (window as any).requestIdleCallback;
      if (typeof requestIdle === 'function') {
        autoMvuReloadReprocessIdleHandle = requestIdle(
          () => {
            autoMvuReloadReprocessIdleHandle = 0;
            void autoReprocessLatestAssistantVariablesIfMissing(reason);
          },
          { timeout: 2500 },
        );
        return;
    ...
    autoMvuReloadReprocessTimer = window.setTimeout(() => {
      autoMvuReloadReprocessTimer = 0;
      runIdleProbe();
    }, 1800);
  }

  async function reprocessLatestAssistantVariables(options?: ReprocessLatestAssistantOptions) {
    ...
    markStageTiming('entry', {
      ...
      source: normalizedOptions.source ?? 'manual',
    });
    const latestAssistant = latestAssistantItem.value;
    if (!latestAssistant || latestAssistant.role !== 'assistant') {
      markStageTiming('skip_no_assistant');
      if (!silent) toastr?.info?.('当前还没有可重新解析变量的 assistant 楼层');
      return;
    }
    if (busy.value) {
      ...
      if (!silent) toastr?.warning?.('正文生成中，等待生成结束后再重试变量解析');
      return;
    }
    ...
    try {
      ...
      if (reprocessResult.status === 'applied') {
        ...
        appendLog(
          'action',
          normalizedOptions.source === 'auto_reload' ? '自动重试额外模型解析' : '重试额外模型解析',
          `已触发 MVU 原生重试额外模型解析：assistant #${latestAssistant.message_id}`,
        );
        if (!silent) toastr?.success?.('已触发 MVU 原生“重试额外模型解析”');
        recordLifecycleTrace(
          'reprocessLatestAssistantVariables',
          'done',
          {
      ...
      if (!silent) toastr?.warning?.(`重试额外模型解析未触发：${reason}`);
    } catch (error) {
      ...
      if (!silent) toastr?.error?.(`重试额外模型解析失败：${message}`);
    } finally {
  ...
  function schedulePluginNativeDomMutationFollowUp(reason: string, messageIds: number[]) {
    const affectedMessageIds = [
      ...new Set(messageIds.map(id => Math.trunc(Number(id))).filter(id => Number.isFinite(id) && id >= 0)),
    ];
    if (affectedMessageIds.length === 0) {
      recordLifecycleTrace('pluginNativeDomMutation', 'follow_up_skipped_no_message_id', { reason });
      return;
    }

    window.setTimeout(() => {
      discoverRecentNativeGalleryImages(reason, affectedMessageIds);
    }, 600);
  }
  ...
  onMounted(async () => {
      ...
      imageGenerationBridge = createImageGenerationEventBridge({
        ...
        onResponseSuccess: ({ requestId, prompt, imageData }) => {
          ...
          const targetMessageIds = [
            ...new Set(
              [matchedResponse?.messageId, recentIntent?.messageId]
                .map(normalizePluginNativeHandoffMessageId)
          ...
          if (responseMessageIds.length > 0) {
            responseMessageIds.forEach(messageId => finishPluginNativePromptHandoff(messageId, 'response_success'));
          ...
          if (isUntargetedResponse) {
            recordLifecycleTrace('imageGenerationEventBridge', 'untargeted_response_no_global_scan', () => ({
              requestId,
              promptHead: prompt.slice(0, 80),
              responseMessageIds,
              pendingTasks: imagePendingTaskManager.getDebugState(),
    ...
    if (isOpeningWorkbenchHostActive()) {
      ...
      if (document.body && typeof MutationObserver !== 'undefined') {
        generatedImageDomObserver = new MutationObserver(records => {
          ...
          if (affectedMessageIds.length === 0) {
            recordLifecycleTrace('pluginNativeDomMutation', 'ready_image_skipped_no_message_id', {
              reason: 'same_layer.plugin_native_dom_mutation',
            });
            return;
          }
      ...
      hostPluginMutationObservers = bindHostPluginMutationObservers(records => {
        ...
        if (affectedMessageIds.length === 0) {
          recordLifecycleTrace('pluginNativeDomMutation', 'ready_image_skipped_no_message_id', {
            reason: 'host.plugin_native_dom_mutation',
          });
          return;
        }
    ...
    rebuildTranscript();
    // 历史载入保持轻量：不在 mounted 阶段预热画廊、拉起宿主同层水合或扫描近期图片。
  ...
  if (typeof document !== 'undefined' && 'addEventListener' in document) {
    ...
    const triggerGalleryRefresh = (reason: string) => {
      ...
      window.setTimeout(() => {
        const visibleMessageIds = collectVisibleAssistantMessageIds();
        if (visibleMessageIds.length === 0) {
          recordLifecycleTrace('visibilityRefresh', 'visibility_refresh_skipped', {
            reason,
            skippedReason: 'no_visible_assistant_message_ids',
          });
          return;
        }
  ...
  onBeforeUnmount(() => {
    ...
    activePluginNativePromptHandoff = null;
    lastPluginNativeLlmImageGenerationSettledAt = 0;
    pluginNativePromptButtonTriggerAt.clear();
    nativeGenerationRevealActive = false;
    releaseHiddenStoryMessagesForNativeGeneration();
    ...
    pluginNativeResponseHydrationSessions.clear();
    pluginNativeResponseHydrationStartedAt.clear();
    galleryInitialCacheTimers.forEach(timer => window.clearTimeout(timer));
  ...
  const loadingInitialGalleryImages = ref(false);
  ...
  const galleryWindowOptions = computed(() => transcriptWindowPages.value);
  const selectedGalleryWindowOption = computed<TranscriptWindowPageOption | null>(
    () => galleryWindowOptions.value.find(option => option.key === selectedGalleryWindowKey.value) ?? null,
  );
  const selectedGalleryWindowMessageIds = computed(() => {
    const option = selectedGalleryWindowOption.value;
    if (!option) return [];
    const ids: number[] = [];
    for (let messageId = option.startId; messageId <= option.endId; messageId += 1) {
      ids.push(messageId);
    }
    return ids;
  });
  const galleryVisibleEntries = computed<GeneratedImageRef[]>(() => {
    const selectedIds = new Set(selectedGalleryWindowMessageIds.value);
    if (selectedIds.size === 0) return [];
    return galleryEntries.value.filter(entry => selectedIds.has(entry.messageId));
  });
  const hasMoreOlderGalleryImages = computed(() => !galleryHistoryExhausted.value);

  function resolveGalleryWindowKey(option: TranscriptWindowPageOption | null | undefined): string {
    return String(option?.key ?? '');
  }

  function resolveDefaultGalleryWindowKey(): string {
    const options = galleryWindowOptions.value;
    const currentPageIndex = transcriptWindowRange.value?.pageIndex ?? 0;
    const currentKey = resolveGalleryWindowKey(options.find(option => option.pageIndex === currentPageIndex));
    const latestKey = resolveGalleryWindowKey(options.find(option => option.isLatest));
    return currentKey || latestKey;
  }

  function normalizeGalleryWindowKey(key: string): string {
    const validKeys = new Set(galleryWindowOptions.value.map(option => option.key));
    return validKeys.has(key) ? key : '';
  }

  function ensureDefaultGalleryWindowSelection(reason = 'gallery.default_window_selection') {
    const normalized = normalizeGalleryWindowKey(selectedGalleryWindowKey.value);
    const nextKey = normalized || resolveDefaultGalleryWindowKey();
    selectedGalleryWindowKey.value = nextKey;
    recordLifecycleTrace('galleryWindowSelection', 'ensure_default', {
      reason,
      selectedWindowKey: nextKey,
      optionCount: galleryWindowOptions.value.length,
    });
  }

  function normalizeSelectedGalleryHydrationMessageIds(messageIds: number[]): number[] {
    const metaIds = new Set(readMessageMetasAfterContainer().map(item => Math.trunc(Number(item.message_id))));
    return [
      ...new Set(
        messageIds.map(id => Math.trunc(Number(id))).filter(id => Number.isFinite(id) && id >= 0 && metaIds.has(id)),
      ),
    ].sort((a, b) => a - b);
  }

  async function hydrateSelectedGalleryWindowMessages(reason: string, messageIds: number[]): Promise<void> {
    const hydrationIds = normalizeSelectedGalleryHydrationMessageIds(messageIds);
    const sessionId = ++galleryWindowHydrationSessionId;
    if (hydrationIds.length === 0) return;

    recordLifecycleTrace('galleryWindowNativeHydration', 'start', {
      reason,
      sessionId,
      messageIds: hydrationIds,
    });

    for (let index = 0; index < hydrationIds.length; index += GALLERY_WINDOW_NATIVE_HYDRATION_CHUNK_SIZE) {
      if (sessionId !== galleryWindowHydrationSessionId) return;
      const chunkIds = hydrationIds.slice(index, index + GALLERY_WINDOW_NATIVE_HYDRATION_CHUNK_SIZE);
      const hiddenChunkIds = readMessageMetasAfterContainer()
        .filter(item => chunkIds.includes(item.message_id) && item.is_hidden === true)
        .map(item => item.message_id);
      const releaseHydrationLease = hostVisualHideController.leaseMessageIdsForPluginNativeHandoff(
        chunkIds,
        `${reason}:gallery_window_native_hydration`,
      );

      try {
        normalizePluginNativeMessageMetadata(chunkIds);
        if (hiddenChunkIds.length > 0) {
          await setChatMessages(
            hiddenChunkIds.map(id => ({ message_id: id, is_hidden: false })),
            { refresh: 'affected' },
          );
        }
        syncPendingRequestHintsFromDom();
        ...
        console.warn('[stream-demo] gallery selected window native hydration failed', {
          reason,
          chunkIds,
          error: error instanceof Error ? error.message : String(error),
        });
        ...
        try {
          if (hiddenChunkIds.length > 0) {
            await setChatMessages(
              hiddenChunkIds.map(id => ({ message_id: id, is_hidden: true })),
              { refresh: 'none' },
            );
          }
        } catch (error) {
          console.warn('[stream-demo] gallery selected window native hydration restore failed', {
            reason,
            chunkIds,
            error: error instanceof Error ? error.message : String(error),
          });
          ...
          releaseHydrationLease();
          queueHidePolicy(`${reason}:gallery_window_native_hydration_done`);
        }
      }
    }
  }
  ...
  function maybeHydrateSelectedGalleryWindowMessages(reason: string, messageIds: number[]) {
    ...
    if (hydrationSignature === lastGalleryWindowHydrationSignature) return;
    lastGalleryWindowHydrationSignature = hydrationSignature;
    void hydrateSelectedGalleryWindowMessages(reason, hydrationIds);
  }

  function scanSelectedGalleryWindow(reason = 'gallery.window_selection') {
    ensureDefaultGalleryWindowSelection(`${reason}:ensure`);
    discoverRecentNativeGalleryImages(reason, selectedGalleryWindowMessageIds.value);
    maybeHydrateSelectedGalleryWindowMessages(reason, selectedGalleryWindowMessageIds.value);
    recordLifecycleTrace('galleryWindowSelection', 'scan', {
      reason,
  ...
  function selectGalleryWindow(key: string) {
    const normalizedKey = String(key ?? '');
    if (!normalizedKey) return;
    const nextKey = normalizeGalleryWindowKey(normalizedKey);
  ...
  function discoverRecentNativeGalleryImages(reason = 'gallery.native_recent_scan', messageIds: number[] = []) {
    ...
    const explicitIds = new Set(explicitMessageIds);
    const messages =
      explicitMessageIds.length > 0
        ? explicitMessageIds
            .map(messageId => {
              const detail = readChatMessageDetail(messageId);
              return detail ? { ...detail, message_id: messageId } : null;
            })
    ...
    if (messages.length === 0) {
      recordLifecycleTrace('galleryNativeRecentScan', 'probe', {
        ...
        candidateCount: 0,
        discovered: 0,
        skippedReason: 'empty-chat',
      });
      return;
    }

    const containerId = getActiveContainerMessageId();
    const knownCurrentIds = new Set(galleryGroups.value.map(group => group.messageId));
    const currentGroupsById = new Map(galleryGroups.value.map(group => [group.messageId, group]));
    const historicalById = new Map(historicalGalleryGroups.value.map(group => [group.messageId, group]));
    const candidates = messages
      .map(message => {
        const messageId = Math.trunc(Number(message?.message_id));
        const rawMessage = resolveHostMessageText(message);
        return { message, messageId, rawMessage };
      })
      ...
      .slice(0, explicitMessageIds.length > 0 ? messages.length : GALLERY_RECENT_NATIVE_SCAN_BATCH_SIZE);
    ...
    for (const { messageId, rawMessage } of candidates) {
      if (knownCurrentIds.has(messageId)) {
        const currentGroup = currentGroupsById.get(messageId);
        if (currentGroup && currentGroup.images.length > 0) {
          historicalById.set(messageId, currentGroup);
          retainedCurrent += 1;
        }
        sampled.push({
          messageId,
          imageCount: currentGroup?.images.length ?? 0,
          knownCurrent: true,
      ...
      if (images.length === 0) continue;
      historicalById.set(messageId, { messageId, images });
      discovered += 1;
    }
    ...
    recordLifecycleTrace('galleryNativeRecentScan', 'probe', {
      ...
      knownCurrentCount: knownCurrentIds.size,
      discovered,
      retainedCurrent,
      sampled: sampled.slice(0, 12),
    });

    if (discovered <= 0 && retainedCurrent <= 0) return;
    historicalGalleryGroups.value = [...historicalById.values()].sort((a, b) => b.messageId - a.messageId);
    galleryRevision.value += 1;
    const discoveryReason = retainedCurrent > 0 && discovered <= 0 ? `${reason}:retained_current` : reason;
    recordLifecycleTrace('galleryNativeRecentScan', 'discovered', {
      reason: discoveryReason,
      mode: scanMode,
      messageIds,
      discovered,
      retainedCurrent,
      historicalGroupCount: historicalById.size,
    });
  }

  function clearGalleryInitialCacheTimers() {
    galleryInitialCacheTimers.forEach(timer => window.clearTimeout(timer));
    galleryInitialCacheTimers.clear();
  }
  ...
  function startGalleryImageCacheSession(reason = 'gallery.drawer_open', mode: GalleryInitialCacheMode = 'drawer') {
    ...
    for (const delayMs of delays) {
      ...
      galleryInitialCacheTimers.add(timer);
    }
  }
  ...
  watch(
    ...
    () => {
      ...
      galleryOlderLastScanHadNoImages.value = false;
      selectedGalleryWindowKey.value = normalizeGalleryWindowKey(selectedGalleryWindowKey.value);
      lastGalleryWindowHydrationSignature = '';
    },
  );

  watch(
    () => transcriptWindowPages.value.map(page => page.key).join('|'),
    () => {
      selectedGalleryWindowKey.value = normalizeGalleryWindowKey(selectedGalleryWindowKey.value);
      lastGalleryWindowHydrationSignature = '';
    },
  ...
  async function loadOlderGalleryImages() {
    ...
    try {
      ...
      recordLifecycleTrace('galleryHistoryScan', 'load_older_probe', {
        reason: 'gallery.load_older',
        scanned,
        nextCursor: cursor,
        discoveredGroupCount: nextGroups.length,
        discoveredImageCount: nextGroups.reduce((total, group) => total + group.images.length, 0),
        discoveredMessageIds: nextGroups.map(group => group.messageId),
        exhausted: galleryHistoryExhausted.value,
        olderZeroHit: galleryOlderLastScanHadNoImages.value,
        historicalGroupCount: historicalGalleryGroups.value.length,
      });
    } finally {
