                        ...
                        h('div', { style: 'text-align:center;padding:30px 20px;color:#999;' }, [
                          ...
                          h(
                            'button',
                            {
                              ...
                              style:
                                'padding:8px 20px;border-radius:8px;border:none;background:#07c160;color:#fff;font-size:13px;font-weight:600;cursor:pointer;',
                            },
                            '📥 从角色卡导入租客',
                          ),
                        ]),
  ...
  async function registerToPhoneSystem(): Promise<void> {
    ...
    PS.on('app-opened', (appId: string) => {
      ...
      const tryMount = (retries = 0) => {
        ...
        const phoneFrame =
          (window.parent.document.getElementById('phone-entry-btn')?.nextElementSibling as HTMLIFrameElement | null) ||
