...
$(() => {
  ...
  function openPhone(): void {
    ...
    phoneIframe.on('load', () => {
      ...
      phoneApp = (Vue as any).createApp(PhoneDesktop);
  ...
  // 聊天切换时也重建手机
  try {
    let chatId = (window.parent as any).SillyTavern?.getContext?.()?.chatId;
    if (typeof (window.parent as any).eventOn === 'function') {
      (window.parent as any).eventOn('CHAT_CHANGED', () => {
        const newId = (window.parent as any).SillyTavern?.getContext?.()?.chatId;
        if (chatId !== newId) {
          chatId = newId;
          destroy();
        }
