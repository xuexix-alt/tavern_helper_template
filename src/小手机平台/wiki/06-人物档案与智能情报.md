...
| `PROFILE_ANALYSIS_SYSTEM_PROMPT` | 角色动态分析专家 + 戏剧人物追踪者：维护「人物弧光档案」——固定本色是出厂设置，档案记录剧情痕迹（关系/态度/经历/性格侧重演变）；**每条结论事件锚定（底色+事件+倾向）**，禁无锚点空泛定性（如「更亲近了」）；**事实优先级：MVU 硬事实 > 最近正文明确事实 > 固定角色世界书 > 当前人物微信 > 上一次动态档案**；只输出 JSON |
| `parseProfileAnalysisOutput(raw, source?)` | 解析（裸 JSON / 围栏 / jsonrepair / 三种包壳）-> zod 校验 -> 若给 source 再校验**人物身份一致** + **证据引用必须在白名单**（引用不存在的证据直接抛错） |
| `buildProfileAnalysisPrompt(source)` | 段落式：分析对象/写作要求（「底色+事件+倾向」事件锚定结构、关系轨迹、防死板防编造）/约束 +【0】**写作对比示例**（few-shot：同一段剧情贯穿 personalityTuning/relationshipInterpretation/storyInteractionSummary/analysisNarrative 四组坏例→好例，坏例即空泛定性黑名单，并附防照抄警告）+【1】MVU 硬事实 +【2】固定世界书 +【3】最近 20 条正文 +【4】微信（context+newlyAdded）+【5】上次动态档案（全部经 `readonlyData()` 包装防注入）+【6】证据标记 +【7】输出 JSON 契约（字段说明含好例示范） |
