...
其他导出：`fetchOpenAiCompatibleModels`（GET `/v1/models` 模型发现，去重冻结返回；端点 404/405 时自动降级尝试不带 `/v1` 的 `/models`）、`openAiModelsEndpoint`（端点规范化：保留代理路径、末段为版本号（v1/v1beta/v2 等）时不重复补 `/v1`、剥离尾部路径、强制 http/https）。
