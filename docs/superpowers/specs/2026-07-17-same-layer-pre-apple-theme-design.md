...
| 主题 | 基础背景 | 系统强调色 | 视觉方向 |
| --- | --- | --- | --- |
| `APPLE-SKY` | `#F2F6FC` | `#007AFF` | 清透天空蓝 |
| `APPLE-MINT` | `#F1F7F5` | `#00C7BE` | 柔和薄荷绿 |
| `APPLE-LAVENDER` | `#F5F2F8` | `#AF52DE` | 低饱和薰衣草紫 |
| `APPLE-SAND` | `#F7F4F0` | `#FF9500` | 温暖沙色橙 |
| `APPLE-ROSE` | `#F8F2F4` | `#FF2D55` | 轻盈玫瑰粉 |
...
首版 color 归档的背景 tint 过强，容易出现“整页染色”或白色浮岛与底色脱节的问题。修订后的规则如下：

- 深色 APPLE 采用接近 `systemBackground` / elevated background 的石墨层：`#111113` → `#1C1C1E` → `#2C2C2E`，避免纯黑和蓝黑渐变。
- 五个浅色变体统一使用接近系统灰阶的中性底色（约 `#F1F2F7`—`#F8F2F4`），浮岛表面回到白色，颜色只保留在主按钮、选中态和极弱环境 wash。
- 所有浅色变体共享 Apple 的正文色 `#1D1D1F`、次要文字 `#6E6E73`、中性分隔线和中性阴影，不再为每个主题单独染色正文层。
- 环境 wash 的 alpha 控制在约 0.05–0.10，玻璃层通过 82%–84% 的表面不透明度、blur 和柔和阴影建立层级。
